...
「X でポスト」は**ボタンではなく素の `<a href>`**。`https://x.com/intent/post?text=…` を
href に載せ、コメント欄の `input` に合わせて張り替える。

リンクにしておくと**ポップアップブロックにも transient activation にも縛られない**。
スクリプトの `window.open` はユーザー操作の有効期限が切れると塞がれるが、
リンクのクリックはブラウザがそのまま通す。

`target` だけ環境で変える。

| | target | 挙動 |
| --- | --- | --- |
| Android | `_self` | Chrome が App Link として X アプリに引き渡す。PWA はその場に残る |
| それ以外 | `_blank` | 新しいタブで x.com の投稿画面 |

Android で `_blank` にすると、X アプリが起動したあとに **x.com のカスタムタブが居残る**
（Web 側は未ログインなのでログイン画面が居座り、毎回閉じることになる）。
`_self` なら引き渡しが成立した時点でナビゲーションが取り消されるので、タブが増えない。

Galaxy Z Fold で実測した4通り。

| 渡し方 | 本文 | 余計なタブ |
| --- | --- | --- |
| `window.open(web)` | 入る | **残る** |
| `twitter://post?message=` | **空** | 出ない |
| `intent://…;package=com.twitter.android;…` | 入る | 出ない |
| 同じウィンドウで web へ（`target="_self"`） | 入る | 出ない ← 採用 |

独自スキームだけが本文を落とす。あとは**タブが残るかどうかだけの差**だった。

X アプリが入っていない場合は、そのまま x.com の intent 画面に進む。

なお **X アプリの投稿画面は、本文の上に空行を1行置いてそこにカーソルを出す。**
渡し方を変えても消えない X 側の仕様で、投稿すると前後の空白としてトリムされる
（実測で確認済み）。見た目だけの話なので追いかけないこと。
